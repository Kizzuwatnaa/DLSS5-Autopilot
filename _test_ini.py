...
print("1) sifirdan kurulum OK ->", techs)
...
ini = R.Ini.load(d / "ReShade.ini")
pre = R.Ini.load(d / "ReShadePreset.ini")
assert ini.get("GENERAL", "EffectSearchPaths") == "." + BS + "ozel" + BS + "**"   # dokunulmadi
assert ini.get("INPUT", "KeyOverlay") == "36,0,0,0"                              # korundu
assert R.split_list(ini.get("GENERAL", "PreprocessorDefinitions")) == ["FOO=1", "DLSS5_MV_PROVIDER=3"]
techs = R.split_list(pre.get("", "Techniques"))
assert techs[0] == "Lumenite_Kernel@lumenite_Kernel.fx", techs   # SAGLAYICI USTTE
assert techs[1] == "DLSS5_Feed@DLSS5_Feed.fx", techs
assert "Clarity@Clarity.fx" in techs                             # kullanicininki duruyor
...
print("2) mevcut ayar korunuyor + saglayici ustte OK ->", techs)

# 3) Virgul kacirma
raw = R.join_list(["A=1", "B=x,y", "C"])
assert raw == "A=1,B=x,,y,C" and R.split_list(raw) == ["A=1", "B=x,y", "C"]
print("3) virgul kacirma OK ->", raw)
...
print("6) saglayici 0 OK ->", techs)

print()
print("TUM INI TESTLERI GECTI")
